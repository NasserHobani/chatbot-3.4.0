...
class MessengerSettingRepository
{
    use RepoResponse;
    ...
    public function messengerSettingUpdate($request)
    {
        DB::beginTransaction();
        ...
        try {
            ...
            if (isset($responseData['error'])) {
                return $this->formatResponse(false, $responseData['error']['message'], 'client.messenger.settings', []);
            } else {
                if (isset($responseData['data']['is_valid']) && $responseData['data']['is_valid'] === true) {
                    ...
                    return $this->formatResponse(false, __('access_token_is_not_valid'), 'client.messenger.settings', []);
                }
            }
            ...
            $clientSetting       = $this->model
                ->where('type', TypeEnum::MESSENGER->value)
                ->where('client_id', Auth::user()->client->id)
                ->first();

            if ($clientSetting) {
                $clientSetting = $this->model->where('type', TypeEnum::MESSENGER)->where('client_id', Auth::user()->client->id)->first();
            ...
            DB::commit();
            return $this->formatResponse(true, __('updated_successfully'), route('client.messenger.settings'), []);
    ...
    public function messengerSync($request)
    {
        DB::beginTransaction();
        try {
            ...
            if (!$clientSetting) {
                return $this->formatResponse(false, __('messenger_setting_not_found'), 'client.messenger.settings', []);
            }
            ...
            if (isset($responseData['data']['is_valid']) && $responseData['data']['is_valid'] === true) {
                ...
                DB::commit();
                return $this->formatResponse(true, __('messenger_settings_synced_successfully'), 'client.messenger.settings', []);
            ...
            return $this->formatResponse(false, $e->getMessage(), 'client.messenger.settings', []);
        }
    }


    public function removeMessengerToken($request, $id)
    {
        if (isDemoMode()) {
            return $this->formatResponse(false, __('this_function_is_disabled_in_demo_server'), 'client.messenger.settings', []);
        }
        DB::beginTransaction();
        try {
            $clientSetting = $this->model->where('type', TypeEnum::MESSENGER)
                ->where('client_id', Auth::user()->client->id)
                ->where('id', $id)
                ->firstOrFail();
            $clientSetting->delete();
            DB::commit();
            return $this->formatResponse(true, __('deleted_successfully'), 'client.messenger.settings', []);
        } catch (\Throwable $e) {
            DB::rollback();
            if (config('app.debug')) {
                dd($e->getMessage());
            ...
            logError('Throwable: ', $e);
            return $this->formatResponse(false, __('an_unexpected_error_occurred_please_try_again_later.'), 'client.messenger.settings', []);
        }
    }
    ...
    private function getPageInfo($accessToken,$clientSetting)
    {
        try {
            ...
            $clientSetting->business_account_name = $responseData['name'];
            $clientSetting->business_account_id = $responseData['id'];
            $clientSetting->update();
        } catch (\Throwable $th) {
            return false;
        }
