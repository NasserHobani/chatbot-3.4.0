...
@section('title', __('messenger_settings'))
@section('content')
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                <h3 class="section-title">{{ __('messenger_settings') }}</h3>
                <div class="default-tab-list default-tab-list-v2  bg-white redious-border p-20 p-sm-30 mb-4">
                    @if (messengerConnected())
                        <div class="alert alert-success" role="alert">
                            {{ __('success_!_you_are_now_connected_to_messenger_cloud_api') }}
                        </div>
                    @else
                        <div class="alert alert-danger" role="alert">
                            {{ __('not_connected!_please_complete_all_the_steps_in_order_to_connect_to_messenger_api') }}
                        </div>
                    @endif
                    
                    @if (!isMessengerAppWebhookConnected())
                        <div class="alert alert-danger" role="alert">
                            <strong>{{ __('oops') }}</strong> {{ __('messenger_webhook_not_connected') }}<br>
                            <small>{{ __('real_time_updates_will_not_be_available_until_the_webhook_is_connected') }}</small>
                            @if (
                                !empty(@Auth::user()->client->messengerSetting->access_token) &&
                                    !empty(@Auth::user()->client->messengerSetting->scopes))
                                <a class="alert-link" href="https://developers.facebook.com/apps/{{ Auth::user()->client->messengerSetting->app_id }}/whatsapp-business/wa-settings/?business_id={{ Auth::user()->client->messengerSetting->business_account_id }}"
                                    target="_blank">
                                    <i class="las la-link"></i> {{ __('add_messenger_webhook') }}
                                </a>
                            @endif
                        </div>
                    @endif
                        ...
                        <div class="col-lg-6">
                            <div class="card h-100">
                                <div class="card-header">
                                    <h5><i class="las la-cloud"></i> {{ __('get_started_with_facebook_official_api') }}</h5>
                                </div>
                                <div class="card-body">
                                    <ul class="list-unstyled">
                                        ...
                                        <li><i class="las la-check-circle"></i>{{ __('3') }}.
                                            {{ __('in_the_messenger_configuration_enter_the_required_information_including_your_webhook_callback_url_and_verify_token') }} {{ __('callback_url') }} and
                                            {{ __('verify_token') }}.</li>
                    ...
                    <form action="{{ url('client/messenger/settings/update') }}" method="POST" enctype="multipart/form-data" class="form">
                        @csrf
                        ...
                        <div class="row mb-3">
                            <div class="col-lg-6">
                                <div class="card h-100">
                                    <div class="card-header">
                                        <h5><i class="las la-key"></i> {{ __('messenger_access_token') }}</h5>
                                    </div>
                                    <div class="card-body">
                                        <div class="mb-4">
                                            ...
                                            <input type="text" class="form-control rounded-2" id="access_token"
                                                name="access_token"
                                                value="{{ isDemoMode() ? '******************' : old('access_token', @Auth::user()->client->messengerSetting->access_token) }}"
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <input type="text" class="form-control rounded-2" id="app_id"
                                                name="app_id"
                                                value="{{ isDemoMode() ? '******************' : old('app_id', @Auth::user()->client->messengerSetting->app_id) }}"
                                                placeholder="{{ __('enter_app_id') }}">
                                            <div class="invalid-feedback nk-block-des text-danger">
                                                <p class="app_id_error error">{{ $errors->first('app_id') }}</p>
                                            </div>
                                        </div>
                                        @if (@Auth::user()->client->messengerSetting)
                                            <div class="p-2 bg-light">
                                                @if (!empty(@Auth::user()->client->messengerSetting->access_token))
                                                    ...
                                                    <div class="mb-2">
                                                        <strong><i class="las la-user-circle"></i>
                                                            {{ __('page_name') }}:</strong>
                                                        <span
                                                            class="text-muted">{{ @Auth::user()->client->messengerSetting->business_account_name }}</span>
                                                    </div>
                                                    <div class="mb-2">
                                                        <strong><i class="las la-user-edit"></i>
                                                            {{ __('page_id') }}:</strong>
                                                        <span
                                                            class="text-muted">{{ Auth::user()->client->messengerSetting->business_account_id }}</span>
                                                    </div>
                                                    ...
                                                    @if (!empty(Auth::user()->client->messengerSetting->expires_at))
                                                        {{-- <div class="mb-2">
                                                    ...
                                                    <?php
                                                    $scopes = @Auth::user()->client->messengerSetting->scopes ?? [];
                                                    ...
                                                    @foreach (Auth::user()->client->messengerSetting->details as $details)
                                                        <div class="mb-3">
                                                            <div class="mb-2">
                                        ...
                                        <div class="d-flex justify-content-end align-items-center mt-30 gap-2">
                                            @if (@Auth::user()->client->messengerSetting->access_token)
                                                <button type="button" class="btn btn-danger text-white __js_delete"
                                                    id="remove_access_token"
                                                    data-id="{{ @Auth::user()->client->messengerSetting->id }}"
                                                    data-url="{{ route('client.messenger.remove-token', @Auth::user()->client->messengerSetting->id) }}">
                                            ...
                                            @if (@Auth::user()->client->messengerSetting)
                                                <button type="button" class="btn btn-secondary" id="sync_button"
                                                    data-id="{{ @Auth::user()->client->messengerSetting->id }}"
                                                    data-url="{{ route('client.messenger-settings.sync', @Auth::user()->client->messengerSetting->id) }}">
