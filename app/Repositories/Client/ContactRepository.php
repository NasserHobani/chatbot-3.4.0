...
class ContactRepository
{
    ...
    public function all($request)
    {
        ...
        return $this->model->latest()->withPermission()->where('assignee_id',auth()->user()->id)->where('type', $request->type)->paginate(setting('pagination'));
    }
    ...
    {
        Log::info("getChatContactListTeamWise");
        Log::info(hasPermission("show_all_contacts"));
        Log::info(auth()->user()->id);
        if (hasPermission("show_all_contacts")){
            $query = $this->model->with(['lastMessage'])
