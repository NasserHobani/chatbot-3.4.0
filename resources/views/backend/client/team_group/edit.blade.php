            ...
            <div class="col-lg-12">
                ...
                <div class="default-tab-list table-responsive default-tab-list-v2 activeItem-bd-md bg-white redious-border p-20 p-sm-30">
                    <div class="default-list-table yajra-dataTable">
                        <form class="form" action="{{ route('client.team-group.update',$team->id) }}"
...
{{--                                                                    <input name="staff_select[]" id="staff_select"--}}
{{--                                                                           type="checkbox"--}}
{{--                                                                           value="{{ $line->user->id }}">--}}
{{--                                                                    <label for="staff_select-{{ $line->user->first_name }}">{{$line->user->first_name}}</label>--}}
                                                                    <label>
                                                                        <input type="checkbox" id="staff_select" name="staff_select[]" {{ in_array($line->id, $team->staff->pluck('id')->toArray() )? 'checked':''}}  value="{{ $line->id }}">
                                                                        <span>{{ $line->user->first_name   }} {{$line->user->last_name}}</span>
                                                                    </label>
                                                                </div>
                                                            </td>
                                                        </tr>
